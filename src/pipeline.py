...
'''The search will not return all the information we're looking for. In order to
 ...
 then can inspect the remaineder for relvance.
 '''
...
'''Alternately, if there are far more channels returned than relevant channels,
we can just select the channels we are interested in'''

relevant_channel_list = ['''channel name 1, channel name 2, ...''']

'''This is fed into the search function from before in order to obtain a the 
channelId's which will be used to get additional information on the channels'''
...
'''gather channel information and format reulting df'''
